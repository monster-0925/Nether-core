...
/** Call node.updateRwSetting() with Comedian 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Comedian 22.x releases try to read these specific settings as
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayComedianUnit")) {
        settings.setValue("DisplayComedianUnit", QVariant::fromValue(ComedianUnit::BTC));
    }
    QVariant unit = settings.value("DisplayComedianUnit");
    if (unit.canConvert<ComedianUnit>()) {
        m_display_bitcoin_unit = unit.value<ComedianUnit>();
    } else {
        m_display_bitcoin_unit = ComedianUnit::BTC;
        settings.setValue("DisplayComedianUnit", QVariant::fromValue(m_display_bitcoin_unit));
    }
...
{
    if (new_unit.isNull() || new_unit.value<ComedianUnit>() == m_display_bitcoin_unit) return;
    m_display_bitcoin_unit = new_unit.value<ComedianUnit>();
    QSettings settings;
    settings.setValue("DisplayComedianUnit", QVariant::fromValue(m_display_bitcoin_unit));
