...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<ComedianUnit>("ComedianUnit");
}
...
ComedianApplication::ComedianApplication()
...
void ComedianApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ComedianApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ComedianGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ComedianApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ComedianApplication::createOptionsModel(bool resetSettings)
{
...
void ComedianApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ComedianGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ComedianGUI::quitRequested, this, &ComedianApplication::requestShutdown);
...
void ComedianApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ComedianApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ComedianApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ComedianApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ComedianApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ComedianApplication::handleRunawayException);
    connect(this, &ComedianApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ComedianApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ComedianApplication::parameterSetup()
{
...
void ComedianApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ComedianApplication::requestInitialize()
{
...
void ComedianApplication::requestShutdown()
{
...
void ComedianApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ComedianGUI::handlePaymentRequest);
        connect(window, &ComedianGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ComedianApplication::handleRunawayException(const QString &message)
{
...
void ComedianApplication::handleNonFatalException(const QString& message)
{
...
WId ComedianApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ComedianApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ComedianGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
