// Copyright (c) 2015-present The Comedian Core developers
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<ComedianAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = ComedianUnits::formatWithUnit(unit, expected_balance, false, ComedianUnits::SeparatorStyle::ALWAYS);
...
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    ComedianAmountField* amountInput = receiveCoinsDialog.findChild<ComedianAmountField*>("reqAmount");
